						   ...
						   UINT  writebuffersize)	// size to the writebuffer
{
	...
	BOOL bResult = FALSE;
	_TCHAR tzPort[MAX_PATH] = { 0 };
	_TCHAR tzBaud[MAX_PATH] = { 0 };
	...
	m_hComm = CreateFile(tzPort,						// communication port string (COMX)
...
//
void CSerialPort::ProcessErrorMessage(LPCTSTR lpErrorText)
{
	CString strTemp = _T("");
	...
	if (lpMsgBuf)
	{
		strTemp.Format(_T("WARNING:  %s Failed with the following error: \r\n%s\nPort: %d\r\n"), lpErrorText, lpMsgBuf, m_nPortNr);
		MessageBox(NULL, strTemp, _T("Application Error"), MB_ICONSTOP);
